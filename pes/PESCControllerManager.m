...
@interface PESCControllerManager ()
...
@implementation PESCControllerManager{
...
+ (id)sharedManager
{
    ...
    static PESCControllerManager *shared = nil;
    if(shared == nil)
    {
        dispatch_once(&onceToken, ^{
            shared = [[PESCControllerManager alloc] init];
...
/**
 ...
 all these are defined in PESCDefines.h
...
- (NSDictionary *)controllerPreferences {
    ...
    if (self.gamePlayDictionary == nil){
        NSString *preferenceFile = @"/var/mobile/Library/Preferences/com.nito.pesc.plist";
        NSString *localFile = [[self documentsFolder] stringByAppendingPathComponent:@"com.nito.pesc.plist"];
...
- (void)startButton {
    ...
    CGPoint start = PAT(kPCActionStartButton);
...
- (PCActionType)actionTypeFromConstant:(NSString *)constant {
    ...
    PCActionType type = kPCActionUndefined;
    if ([constant isEqualToString:PCActionThroughOrPress]){
        type = kPCActionThroughOrPress;
    } else if ([constant isEqualToString:PCActionClearOrShot]){
        type = kPCActionClearOrShot;
    } else if ([constant isEqualToString:PCActionSwitchOrPass]){
        type = kPCActionSwitchOrPass;
    } else if ([constant isEqualToString:PCActionDash]){
        type = kPCActionDash;
    } else if ([constant isEqualToString:PCActionStartButton]){
        type = kPCActionStartButton;
    }
    return type;
...
- (CGPoint)pointForActionType:(PCActionType)type {
    ...
    */
    ...
    switch (type) {
        case kPCActionDash:
            outpoint = [self convertPointForScreen:CGPointMake(679,350)];
            break;
            
        case kPCActionClearOrShot:
            outpoint = [self convertPointForScreen:CGPointMake(681,245)];
            break;
            
        case kPCActionSwitchOrPass: //lay down
            outpoint = [self convertPointForScreen:CGPointMake(560,359)];
            break;
            
        case kPCActionThroughOrPress:
            outpoint = [self convertPointForScreen:CGPointMake(588,268)];
            break;
